...
+
Si vous êtes un utilisateur existant de SnapCenter, vous devez migrer vos sauvegardes et métadonnées existantes de machines virtuelles et de banques de données SnapCenter. Pour obtenir des informations sur la migration, consultez link:scpivs44_migrate.html#migration-options["Options de migration"^].
